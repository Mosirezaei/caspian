...
export default function ConsultModal({ isOpen, onClose }) {
  ...
  const handleClose = () => {
    onClose();
    setTimeout(() => { setSuccess(false); setForm({ name: '', phone: '', contact: '', service: '', notes: '' }); }, 300);
  };
      ...
      {isOpen && (
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
            <motion.div initial={{ opacity: 0, scale: 0.93, y: 24 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.93, y: 24 }} transition={{ type: 'spring', damping: 22, stiffness: 300 }} dir={isRtl ? 'rtl' : 'ltr'} className="w-full max-w-md glass-panel rounded-3xl border border-primary/25 shadow-2xl pointer-events-auto" onClick={e => e.stopPropagation()}>
              <div className="flex items-start justify-between px-6 pt-5 pb-4 border-b border-white/10">
                <div><h2 className="text-lg font-black gold-gradient-text">{t.title}</h2><p className="text-xs text-foreground/50 mt-0.5">{t.subtitle}</p></div>
                <button onClick={handleClose} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-white/10 transition-colors text-foreground/40 hover:text-foreground mt-0.5"><X className="w-4 h-4" /></button>
              </div>
              <div className="px-6 py-5">
                {success ? (
                  <div className="text-center py-8"><CheckCircle className="w-14 h-14 text-green-500 mx-auto mb-4" /><h3 className="text-base font-bold text-foreground mb-1">{lang === 'fa' ? 'درخواست ثبت شد!' : 'Request Submitted!'}</h3><button onClick={handleClose} className="mt-6 px-6 py-2 rounded-xl bg-gradient-to-r from-primary to-primary/80 text-background font-bold text-sm">بستن</button></div>
                ) : (
